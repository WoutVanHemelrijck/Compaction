...
export interface LogEntry {
  ...
  /** start idx (inclusive) for what part of logs to drop */
  startIdx?: number;
  /** final idx (inclusive) for what part of logs to drop */
  finalIdx?: number;
}
...
export function validateLogEntry(entry: LogEntry): void {
  ...
  if (entry.type === LogEntryType.COMMAND) {
    ...
    // no fields
  } else if (entry.type === LogEntryType.DROPLOG) {
    if (!entry.startIdx || !entry.finalIdx) {
      throw new Error(`Invalid DROPLOG: startIdx=${entry.startIdx} or finalIdx=${entry.finalIdx} was undefined`);
    }
    if (entry.startIdx > entry.finalIdx) {
      throw new Error(`Invalid DROPLOG: startIdx must be <= finalIdx, but it was not.`);
    }
  } else {
    throw new Error(
      `Invalid log entry type: ${String(entry.type)}. Type must be either COMMAND, CONFIG, NOOP, or DROPLOG`,
    );
  }
}
