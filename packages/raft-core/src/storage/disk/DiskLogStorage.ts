...
export class DiskLogStorage implements LogStorage {
  ...
  async debug_getAllLogEntries(): Promise<LogEntry[]> {
    ...
    // [DEBUG]
    //console.log(this.offsetIndex);
    console.log('Printing ALL log entries...');
    //

    const idxs = Array.from(this.offsetIndex.keys());
    const min_idx = Math.min(...idxs);
    const max_idx = Math.max(...idxs);
    //
  ...
  async compact(upToIndex: number, term: number): Promise<void> {
    ...
    await this.lock.runExclusive(async () => {
      ...
      const combined = records.length > 0 ? Buffer.concat([header, ...records]) : header;
  ...
  private async readRecord(offset: number): Promise<LogEntry> {
    ...
    try {
      ...
      if (storedCrc !== computedCrc) {
        throw new StorageError(`WAL CRC32 mismatch at offset ${offset}: stored=${storedCrc}, computed=${computedCrc}`);
      }
  ...
  private async loadAndScan(): Promise<void> {
    ...
    try {
      ...
      while (true) {
        ...
        const crcData = Buffer.concat([prefix.subarray(4), rest]);
        if (crc32(crcData) !== storedCrc) {
          truncateAt = offset;
          break;
        }

        const index = StorageNumberUtil.bigIntToSafeNumber(
          rest.readBigInt64BE(0),
          `wal entry index at offset ${offset}`,
        );
